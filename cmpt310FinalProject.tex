...
The algorithm proposed in [1] includes the concerns of random delays being considered
in the algorithm. This is accomplished by the agent applying a buffer of actions on the
environment at a given time. This allows the agent to supply multiple actions in case of a 
large delay occurring. However, if observations arrive from the environment this buffer of 
actions will be updated based upon the new observations received.

The git repository provided by [1] included wrappers around the Mujoco [reference] environments.
These wrappers add various different types of random delays to both actions and observations to/
from the environment. 
There are different type of wrappers for different delays including real world measured 
internet delays.
When we implemented our own version of the code we reused these wrappers.

\subsection{Algorithm Description}

The algorithm proposed keeps track of a buffer of actions ...
...
\section{Recreating The Algorithm}

To-do, time to code some shit 
...
We would like to acknowledge -- PROF and TA I guess
